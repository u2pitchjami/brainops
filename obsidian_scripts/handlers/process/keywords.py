...
def process_and_update_file(filepath):
    ...
    logger.debug(f"[DEBUG] process_and_update_file : vérif du fichier de mots clés is_file_update ")
    file_updated, new_modified_time = is_file_updated(keywords_file, LAST_MODIFIED_TIME)
    if file_updated:
        print("[INFO] Rechargement des mots-clés...")
        logger.debug(f"[DEBUG] process_and_update_file : fichier modifié : rechargement des  mots clés")
    ...
    logger.debug(f"[DEBUG] Contenu brut après extract_yaml_header CONTENT : {content_lines[:5]}")
    
    # Charger les mots-clés depuis le fichier
    TAG_KEYWORDS = load_keywords(keywords_file)
    #data = yaml.safe_load(f)
    logger.debug(f"[DEBUG] process_and_update_file : Contenu du fichier YAML chargé : {TAG_KEYWORDS}")
    # Analyser les sections et générer des tags
    logger.debug(f"[DEBUG] process_and_update_file : envoie vers tag_sections")
    tagged_sections = tag_sections(content_lines)

    # Réécrire le contenu dans le fichier
    logger.debug(f"[DEBUG] process_and_update_file : envoie vers integrate_tags_in_file")
...
def is_file_updated(file_path, last_modified_time):
    logger.debug(f"[DEBUG] entrée dans is_file_updated")
    try:
        current_modified_time = os.path.getmtime(file_path)
        logger.debug(f"[DEBUG] is_file_updated : nouvelle date : {current_modified_time}")
...
def extract_sections(content_lines):
    ...
    logger.debug(f"[DEBUG] entrée fonction : extract_sections")
...
def detect_tags_in_text(content_lines, TAG_KEYWORDS):
    logger.debug(f"[DEBUG] entrée fonction : detect_tags_in_text")
...
def tag_sections(content_lines):
    logger.debug(f"[DEBUG] entrée fonction : tag_sections")
    """Analyse chaque section et génère des tags basés sur le titre et le contenu."""
    logger.debug(f"[DEBUG] tag_sections : envoie vers extract_sections")
    sections = extract_sections(content_lines)
    tagged_sections = []

    for section in sections:
        # Séparer le titre de la section du contenu
        logger.debug(f"[DEBUG] tag_sections : Séparer le titre de la section du contenu")
        ...
        logger.debug(f"[DEBUG] tag_sections : envoie vers detect_tags_in_text pour title_tags")
        title_tags = detect_tags_in_text(title, TAG_KEYWORDS)
        #logger.debug(f"[DEBUG] tag_sections : title_tags : {title_tags}")
        logger.debug(f"[DEBUG] tag_sections : envoie vers detect_tags_in_text pour content")
...
def integrate_tags_in_file(filepath, tagged_sections, header_lines):
    ...
    with open(filepath, "w", encoding="utf-8") as file:
        ...
        if header_lines:  # Vérifie que l'entête n'est pas vide
            ...
            logger.debug(f"[DEBUG] integrate_tags_in_file : Entête YAML correctement écrite : {''.join(header_lines)}")
        ...
        for section in tagged_sections:
            ...
            logger.debug(f"[DEBUG] integrate_tags_in_file : title : {section['title']} : {filepath}")
            ...
            logger.debug(f"[DEBUG] integrate_tags_in_file : section content : {section['content']}")
