...
def update_note(note_id, dest_path, src_path=None):
    ...
    logger.debug(f"[DEBUG] ===== Entrée update_note | dest = {dest_path}, src = {src_path}")
    categ_trigger = False
    ...
    try:
        ...
        title_yaml = metadata.get("title", Path(dest_path).stem.replace("_", " ").replace(":", " "))
        ...
        category_name, subcategory_name, category_id_dest, subcategory_id_dest = categ_extract(base_folder)
        logger.debug(f"[DEBUG] {category_name}, {subcategory_name}, {category_id_dest}, {subcategory_id_dest}")
        ...
        if category_id != category_id_dest or subcategory_id != subcategory_id_dest:
            ...
            logger.debug(f"[DEBUG]Nouvelle catégorie : {category_name}, Nouvelle sous-catégorie : {subcategory_name}, Nouveau folder_id : {folder_id}")
            categ_trigger = True
        ...
        logger.info(f"[INFO] Note mise à jour avec succès : {dest_path} (note_id={note_id})")
