...
def make_embeddings_synthesis(note_id, filepath):
    ...
    final_response = call_ollama_with_retry(prompt, model_ollama="llama3.1:8b-instruct-q8_0")
...
def build_summary_prompt(blocks, structure="simple") -> str:
    ...
    """
    logger.debug(f"[DEBUG] build_summary_prompt")
    intro = (
        "Voici des extraits importants d'une note. Résume-les de façon concise et claire.\n\n"
        if structure == "simple"
        else "Voici plusieurs extraits pertinents issus d'une note. Organise les idées par thème, puis fais une synthèse claire.\n\n"
    )
