...
#         try:
#             if Path(to_abs(str(p))).is_dir() and not _is_hidden_path(p):
#                 yield Path(to_rel(p))
...
#     Path(to_abs(out_dir)).mkdir(parents=True, exist_ok=True)
#     filename = Path(to_abs(out_dir)) / f"{prefix}_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.csv"
...
#             cursor.execute(
#                 f"SELECT id, {folders_path_col} AS path, folder_type, category_id,
#                   subcategory_id FROM obsidian_folders"
#             )
...
#         db_note_paths_resolved = {p for p in db_note_paths if p.exists()}
...
#                     conn.commit()
#                 logger.info(
#                     "%s Réparation parent_id: synthesis %s <-> archive %s",
#                     "APPLY" if apply else "DRY-RUN", s_id, a_id
#                 )
...
#             s_classification = get_category_context_from_folder(folder_path=s_dir.parent.as_posix(), logger=logger)
#             a_classification = get_category_context_from_folder(folder_path=a_dir.parent.as_posix(), logger=logger)

#             upd_s = (s.get("category_id") != s_classification.category_id) or (
#                 s.get("subcategory_id") != s_classification.subcategory_id
#             )
#             upd_a = (a.get("category_id") != a_classification.category_id) or (
#                 a.get("subcategory_id") != a_classification.subcategory_id
#             )
#             if upd_s or upd_a:
#                 if apply:
#                     with conn.cursor() as cur:
...
#                                 (s_classification.category_id, s_classification.subcategory_id, s_id),
#                             )
#                         if upd_a:
#                             cur.execute(
#                                 "UPDATE obsidian_notes SET category_id=%s, subcategory_id=%s WHERE id=%s",
#                                 (a_classification.category_id, a_classification.subcategory_id, a_id),
#                             )
#                     conn.commit()
#                 logger.info(
...
#                     s_id,
#                     s_classification.category_id,
#                     s_classification.subcategory_id,
#                     a_id,
#                     a_classification.category_id,
#                     a_classification.subcategory_id,
#                 )
...
#         help="Ne pas dispatcher les anomalies bloquantes vers handle_errored_file"
#     )
#     # Auto-fix non bloquants
#     parser.add_argument(
#         "--fix-non-blocking",
...
#         logger.info("SCRIPT %s (mtime=%s)", Path(__file__), time.ctime(Path(__file__).stat().st_mtime))
...
# def main() -> int:
#     args = parse_args()
#     base_path = Path(args.base_path or BASE_PATH)
#     out_dir = Path(args.out_dir or LOG_FILE_PATH)
