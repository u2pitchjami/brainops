...
def process_android_datas():
    ...
    try:
        ...
        cursor.execute(
            "SELECT DISTINCT execution_timestamp FROM android_tmp ORDER BY execution_timestamp ASC;"
        )
        ...
        for row in timestamps:
            ...
            if (
                prev_execution_timestamp
                and execution_timestamp.date() == prev_execution_timestamp.date()
            ):
                period_start = prev_execution_timestamp
            else:
                period_start = execution_timestamp - timedelta(
                    minutes=10
                )  # Assumer qu'on suit un pas de 10 min
                logger.info(
                    f"🌙 Nouveau jour détecté, period_start initialisé à {period_start}"
                )
            ...
            cursor.execute(
                "SELECT COUNT(*) as count FROM android_usage WHERE timestamp = %s;",
                (execution_timestamp,),
            )
            ...
            if result["count"] == 0:
                ...
                if active_entries:
                    ...
                    logger.info(
                        f"🔍 Aucune donnée active à insérer pour {execution_timestamp}"
                    )
            else:
    ...
    try:
        ...
        if last_timestamp:
            ...
            conn.commit()
            logger.info(
                f"🗑️ Suppression des entrées de android_tmp antérieures à {last_timestamp - timedelta(hours=12)}."
            )
        else:
            logger.info(
                "ℹ️ Aucune donnée dans android_usage, pas de purge de android_tmp."
            )
    except mysql.connector.Error as e:
