...
print(
    f"🔍 Niveau du root logger largenotegpt : {logging.getLevelName(root_logger.level)}"
)
print(
    f"🔍 Nombre de handlers dans root logger largenotegpt : {len(root_logger.handlers)}"
)
...
def split_large_note(content, max_words=1000):
    logger.debug("[DEBUG] entrée split_large_note")
    """
...
def process_large_note_gpt_test(content, filepath, model_ollama):
    logger.info("[DEBUG] entrée process_large_note")
    """
    ...
    logger.debug(
        f"[DEBUG] Contenu brut avant extract_yaml_header : {repr(content[:100])}"
    )
    try:
        ...
        with open(logfile, "a", encoding="utf-8") as f:
            f.write(
                f"============= [DEBUG] process_large_note : MODEL {model_ollama} =============\n"
            )  # Ajoute un titre

        for i, block in enumerate(blocks):
            print(f"[INFO] Traitement du bloc {i + 1}/{len(blocks)}...")
            logger.debug(
                f"[DEBUG] process_large_note : Traitement du bloc {i + 1}/{len(blocks)}"
            )
            ...
            with open(logfile, "a", encoding="utf-8") as f:
                f.write(
                    f"=== [DEBUG] process_large_note : PROMPT {i + 1}/{len(blocks)} ===\n"
                )  # Ajoute un titre
                f.write(prompt + "\n\n")  # On ajoute une nouvelle ligne

            logger.info("[DEBUG] process_large_note : envoi vers ollama")
            try:
            ...
            logger.debug(
                f"[DEBUG] process_large_note {i + 1}/{len(blocks)} : réponse {response}"
            )
            ...
            with open(logfile, "a", encoding="utf-8") as f:
                f.write(
                    f"=== [DEBUG] process_large_note : REPONSE {i + 1}/{len(blocks)} ===\n"
                )
                f.write(response + "\n\n")  # On ajoute une nouvelle ligne

            logger.debug(
                "[DEBUG] process_large_note : retour ollama, récupération des blocs"
            )
        ...
        logger.debug("[DEBUG] process_large_note : mis à jour du fichier")
...
def split_large_note_by_titles(content):
    """
    Découpe une note en blocs basés sur les titres (#, ##, ###), en gérant une éventuelle introduction avant le premier
    titre.
    ...
    # Cas 2 : Aucun titre trouvé, tout le contenu est une introduction
        ...
        intro = content.strip()
        logger.debug(
            f"[DEBUG] Aucun titre trouvé, tout le contenu est traité comme une introduction : {intro[:50]}"
        )
        if intro:
            blocks.append(f"## **Introduction**\n\n{intro}")
...
def split_large_note_by_titles_and_words_gpt_test(content, word_limit=1000):
    """
    Découpe une note en blocs basés sur les titres (#, ##, ###), et regroupe les sections en paquets de 1000 mots
    maximum.
