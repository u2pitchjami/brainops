...
def regen_synthese_from_archive(note_id, filepath=None):
    ...
    """
    logger.info(
        f"[REGEN] Démarrage régénération synthèse pour note_id={note_id} filepath={filepath}"
    )
...
def regen_header(note_id, filepath, parent_id=None):
    ...
    try:
        ...
        logger.info(
            f"[REGEN_HEADER] En-tête régénérée pour {filepath} avec statut {status}"
        )
...
def force_categ_from_path(filepath, note_id):
    ...
    try:
        ...
        category_name, subcategory_name, category_id, subcategory_id = categ_extract(
            base_folder
        )
        ...
        logger.debug(
            f"[DEBUG] process_single_note mise à jour base de données : {updates}"
        )
        ...
        logger.info(
            f"[FORCE_CATEG] Catégorisation et synthèse terminées pour {new_path}"
        )
