        ...
        for i, block in enumerate(blocks):
            ...
            if persist_blocks:
                try:
                    ...
                    if row:
                        ...
                        if status == "processed" and resume_if_possible:
                            ...
                            if isinstance(response, list):
                                logger.debug(
                                    f"[DEBUG] Bloc {i + 1} contient (liste):\n"
                                    + "\n---\n".join(map(str, response))
                                )
                                response = "\n".join(map(str, response))
                            elif not isinstance(response, str):
                                logger.warning(
                                    f"Bloc {i + 1} : réponse inattendue \
                                        ({type(response)}), tentative de conversion en string"
                                )
                    ...
                    logger.error(
                        f"[ERROR] Insertion bloc {i} échouée pour {filepath} : {e}"
                    )
            ...
            if source == "embeddings":
                ...
                try:
                    ...
                    logger.error(
                        f"[ERROR] Bloc {i + 1} : échec de conversion JSON : {e}"
                    )
                    response = "null"
            else:
                if isinstance(response, list):
                    ...
                    logger.warning(
                        f"Bloc {i + 1} : réponse inattendue ({type(response)}), tentative de conversion en string"
                    )
                    response = str(response)

            if persist_blocks:
                update_bloc_response(
                    note_id, filepath, block_index, response, source, status="processed"
                )
