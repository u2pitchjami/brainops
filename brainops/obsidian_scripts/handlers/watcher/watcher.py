...
from brainops.obsidian_scripts.handlers.watcher.queue_manager import (
    enqueue_event,
    log_event_queue,
    process_queue,
)
...
def start_watcher() -> None:
    ...
    while True:
        ...
        if now - last_purge > 3600:
            ...
            logger.info(
                "[MAINTENANCE] Lancement de la purge des locks expirés (timeout=7200s)"
            )
        ...
        if today != last_rollover_date:
            for handler in logger.handlers:
                if isinstance(handler, TimedRotatingFileHandler):
                    logger.info(
                        "🌀 Rollover log déclenché automatiquement dans watcher."
                    )
                    handler.doRollover()
...
class NoteHandler(FileSystemEventHandler):
    ...
    def on_moved(self, event):
        if not self.is_hidden(event.src_path) and not self.is_hidden(event.dest_path):
            event_type = "directory" if event.is_directory else "file"
            logger.info(
                f"[INFO] [DÉPLACEMENT] {event_type.upper()} → {event.src_path} -> {event.dest_path}"
            )
