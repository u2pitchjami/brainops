...
from brainops.obsidian_scripts.handlers.watcher.queue_utils import (
    PendingNoteLockManager,
    get_lock_key,
)
...
def enqueue_event(event) -> None:
    if event["type"] == "file":
        ...
        logger.debug(
            f"[CHECK] Enrichi note_id: {note_id}, set: {pending_note_ids}, key_type: {type(note_id)}"
        )
...
def process_queue() -> None:
    from brainops.obsidian_scripts.handlers.start.process_folder_event import (
        process_folder_event,
    )
    from brainops.obsidian_scripts.handlers.start.process_single_note import (
        process_single_note,
    )
    ...
    while True:
        try:
            ...
            if not wait_for_file(file_path) and (event["action"] != "deleted"):
                logger.warning(
                    f"⚠️ Fichier introuvable, suppression de l'événement : {file_path}"
                )
            ...
            if event["type"] == "file":
                if event["action"] in ["deleted"]:
                    logger.debug("[DEBUG] ===== Event deleted")
                ...
                if note_id:
                    if event["action"] in ["created", "modified"]:
                        logger.debug(
                            f"[BUSY] Notes en cours : {list(pending_note_ids)}"
                        )
                        logger.debug(
                            f"[DEBUG] ===== Event created, modified : {event['action']}"
                        )
                        ...
                        logger.debug("[DEBUG] ===== Event moved")
                        logger.debug(
                            f"[BUSY] Notes en cours : {list(pending_note_ids)}"
                        )
                        ...
                        if not psn:
                            ...
                            logger.debug(
                                f"[DEBUG] Sortie process_note_event moved note_id : {note_id}"
                            )
                        continue
                continue
                ...
                if event["action"] == "moved":
                    ...
                    logger.debug(
                        f"📂 [DEBUG] Action: {event['action']} | Path: {event['path']}"
                    )
                    process_folder_event({"action": "created", "path": event["path"]})
                    logger.debug("[DEBUG] Sortie Created, entrée Deleted")
                    process_folder_event(
                        {"action": "deleted", "path": event["src_path"]}
                    )
                    logger.debug("[DEBUG] fini")
                else:
                    process_folder_event(event)

            if event:
                logger.debug(
                    f"[DEBUG] Traitement terminé pour {event['type']} - {event['action']}"
                )
            else:
            ...
            logger.error(
                f"[ERREUR] Erreur dans le traitement de la file d'attente : {e}"
            )
