...
def path_is_inside(base: str | Path, target: str | Path) -> bool:
    ...
    try:
        return (
            ...
            .is_relative_to(Path(base).resolve(strict=False))
        )
        ...
        try:
            return (
                ...
                in Path(target).resolve(strict=False).parents
            )
        except Exception:
            return False
    except Exception:
...
def get_relative_parts(folder_path: str | Path, base_path: str | Path):
    ...
    try:
        ...
        logger.warning(
            f"[WARN] get_relative_parts : {folder_path} n'est pas dans {base_path}"
        )
        return None
