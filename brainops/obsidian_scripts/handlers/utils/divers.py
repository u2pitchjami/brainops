...
def make_relative_link(original_path, filepath):
    ...
    if synt_path in original_path.parents:
        ...
        raise ValueError(
            f"Le fichier {original_path} est hors du répertoire de base {synt_path}"
        )
...
def prompt_name_and_model_selection(note_id, key, forced_model=None):
    logger.debug(
        "[DEBUG] prompt_name_selection note_id: %s, key: %s, forced_model: %s",
        note_id,
        key,
        forced_model,
    )
    ...
    logger.debug(
        "[DEBUG] Langue détectée : %s → prompt: %s, modèle: %s",
        lang,
        prompt_name,
        model_ollama,
    )
...
def should_trigger_process(
    note_id: int, new_word_count: int, threshold: int = 100
    ...
    logger.debug(
        f"[trigger_check] Note {note_id} | status: {status} |\
            old_word_count: {old_word_count} |word_diff: {word_diff} | parent_id: {parent_id}"
    )
