...
from brainops.obsidian_scripts.handlers.utils.divers import make_relative_link
...
def process_import_syntheses(filepath, note_id):
    ...
    try:
        ...
        if not parent_id:
            logger.debug("[DEBUG] process_import_syntheses lancement copy_to_archives")
        ...
        logger.debug(
            f"[DEBUG] process_import_syntheses : original_path {original_path}"
        )
        ...
        questions = make_questions(
            filepath=archive_path, note_id=note_id, content=final_response
        )
        ...
        logger.debug(
            f"[DEBUG] process_import_syntheses : envoi vers make_properties {filepath} "
        )
