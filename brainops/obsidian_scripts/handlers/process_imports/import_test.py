...
def process_class_imports_test(filepath, note_id):
    ...
    destination_path = (
        "/mnt/user/Documents/Obsidian/notes/Z_technical/output_tests_imports/"
    )
    filename = os.path.basename(filepath)  # Extrait "fichier.txt"
    logger.debug(f"[DEBUG] filename : {filename}")
    # models_ollama = ["mixtral:8x7b-instruct-v0.1-q5_K_M", "mistral:7B-Instruct",\
    # "mixtral:latest", "mistral:latest", "llama3:8b-instruct-q6_K","llama-summary-gguf:latest",
    # "qwen2.5:14b", "llama-chat-summary-3.2-3b:latest", "llama3.2-vision:11b", "deepseek-r1:14b",
    # "llama3.2:latest", "llama3:latest"]  # Liste des modèles à tester
    models_ollama = [
        "mistral:7B-Instruct",
        "mistral:latest",
    ...
    for model in models_ollama:
        ...
        new_filename1 = f"{os.path.splitext(filename)[0]}_\
            {safe_model_name}{os.path.splitext(filename)[1]}"
        ...
        process_large_note(
            new_file_path2, entry_type="divers", model_name=model, source="other2"
        )
        ...
        if isinstance(response, str):
            ...
            body_content = "\n".join(
                block.strip() for block in response if isinstance(block, str)
            ).strip()
        else:
