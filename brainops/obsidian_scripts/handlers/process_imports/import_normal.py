...
from brainops.obsidian_scripts.handlers.utils.files import copy_file_with_date
...
def pre_import_normal(filepath, note_id):
    ...
    try:
        ...
        logger.debug(
            f"[DEBUG] pre_import_normal fin get_note_type new_path : {new_path}"
        )
        filepath = new_path
        ...
        logger.debug(
            f"[DEBUG] pre_import_normal mise à jour base de données : {updates}"
        )
        ...
        if "gpt_import" in base_folder:
            logger.info(
                f"[INFO] Conversation GPT détectée, déplacement vers : {base_folder}"
            )
            psn = True
            return psn
...
def import_normal(filepath, note_id):
    ...
    try:
        ...
        logger.debug(
            f"[DEBUG] import_normal : import normal envoi vers make_properties {filepath}"
        )
