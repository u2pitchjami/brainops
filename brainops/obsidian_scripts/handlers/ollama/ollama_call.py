...
def call_ollama_with_retry(
    prompt: str, model_ollama: str, retries: int = 5, delay: int = 10
) -> str | None:
    """
    ...
    for i in range(retries):
        try:
            ...
            if i < retries - 1:
                ...
                logger.error(
                    "[ERREUR] Ollama ne répond pas après plusieurs tentatives."
                )
                raise
...
def ollama_generate(prompt: str, model_ollama: str) -> str | None:
    logger.debug("[DEBUG] entrée fonction : ollama_generate")
    ...
    logger.debug(
        f"[DEBUG] ollama_generate, ollama_url_generate : {ollama_url_generate}"
    )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            raise OllamaError(
                f"[ERREUR] Réponse inattendue d'Ollama : {response.status_code}"
            )
