...
def is_folder_included(path, include_types=None, exclude_types=None):
    ...
    result = safe_execute(
        cursor, "SELECT folder_type FROM obsidian_folders WHERE path = %s", (path,)
    ).fetchone()
...
def get_path_from_classification(category_id, subcategory_id=None):
    ...
    """
    logger.debug(
        "[DEBUG] get_path_from_classification pour catégorie: %s, sous-catégorie: %s",
        category_id,
        subcategory_id,
    )
    conn = get_db_connection()
    ...
    if subcategory_id:
        result = safe_execute(
            cursor,
            "SELECT id, path FROM obsidian_folders WHERE category_id = %s AND subcategory_id = %s LIMIT 1",
    ...
    logger.warning(
        "[WARN] Aucun dossier trouvé pour la catégorie: %s et sous-catégorie: %s",
        category_id,
        subcategory_id,
    )
    return None
