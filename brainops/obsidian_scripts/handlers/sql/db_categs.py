...
def delete_category_from_db(category_name, subcategory_name=None):
    ...
    if subcategory_name:
        cursor.execute(
            "DELETE FROM obsidian_categories WHERE name = %s\
        ...
        cursor.execute(
            "DELETE FROM obsidian_categories WHERE name = %s\
                AND NOT EXISTS (SELECT 1 FROM obsidian_categories WHERE parent_id = obsidian_categories.id)",
            (category_name,),
        )
...
def get_path_safe(note_type, filepath, note_id):
    ...
    try:
        ...
        if not subcategory_result:
            logger.info(
                f"[INFO] Sous-catégorie absente : {subcategory}. Création en cours..."
            )
        ...
        logger.debug(
            f"[DEBUG] Sous-catégorie {subcategory_id} , categ : {category_id}."
        )
...
def add_dynamic_subcategory(category, subcategory):
    ...
    if not category_result:
        logger.warning(
            f"[WARN] Impossible d'ajouter la sous-catégorie {subcategory}, la catégorie {category} est absente."
        )
        conn.close()
        return None
    ...
    cursor.execute(
        "SELECT id, path FROM obsidian_folders WHERE category_id = %s AND subcategory_id IS NULL",
        (category_id,),
    )
    folder_category_result = cursor.fetchone()

    if not folder_category_result:
        logger.warning("[WARN] Impossible folder_category_result est absente.")
        ...
        result = safe_execute(
            cursor, "SELECT id FROM obsidian_folders WHERE path = %s", (Z_STORAGE_PATH,)
        ).fetchone()
...
def add_dynamic_category(category):
    ...
    """
    logger.debug("add_dynamic_category")
    ...
    if not folder_parent_id:
        logger.warning("[WARN] Impossible folder_parent_id est absent.")
        conn.close()
        return None
