...
def add_folder_to_db(
    ...
    # Vérif d'existence
    result = safe_execute(
        cursor, "SELECT id FROM obsidian_folders WHERE path = %s", (str(folder_path),)
    ).fetchone()
    ...
    logger.debug(
        f"[DEBUG] folder_name : {folder_name}, str(folder_path) : {str(folder_path)},\
            folder_type : {folder_type}, parent_id : {parent_id}, category_id : {category_id}, subcategory_id : {subcategory_id}"
    )
    cursor.execute(
        """
...
def delete_folder_from_db(folder_path: str) -> bool:
    ...
    try:
        ...
        safe_execute(
            cursor, "DELETE FROM obsidian_folders WHERE path = %s", (folder_path,)
        )
        conn.commit()
