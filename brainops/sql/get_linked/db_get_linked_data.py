    ...
    try:
        ...
        with get_dict_cursor(conn) as cur:
            ...
            if what == "category":
                ...
                logger.debug(f"[DEBUG] get_folder_linked_data category_id: {cat_id} - {type(cat_id)}")
                if cat_id:
                    try:
                        cursor.execute(
                        ...
                        logger.error("Erreur lors de la récupération de la catégorie : %s", e)
                        return {}
                return {}
